 ...
 if(orderDetails != []){
   var AccountId = '00106000023CXXzAAO';
    for(let i in orderDetails){
      if(orderDetails[i].PurchaseDate != ""){
      var date = orderDetails[i].PurchaseDate.getDate();
      var month = orderDetails[i].PurchaseDate.getMonth(); //Be careful! January is 0 not 1
      var year = orderDetails[i].PurchaseDate.getFullYear();
      orderDetails[i].PurchaseDate = date + "-" +(month + 1) + "-" + year;  
      }
